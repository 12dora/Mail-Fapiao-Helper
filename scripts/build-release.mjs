...
// Production boundary (审计 OCR-11 同类问题的兜底)。测试用的 mock OCR 与故障注入
// 钩子只在“可证明的开发树”里才启用，判据之一是 dist/buildInfo.js 的 production 标记。
// npm 的 pack/dist 脚本会写这个标记，但直接 `node scripts/build-release.mjs` 会绕过它，
// 那样打出来的包会带着可激活的测试钩子。打包入口在这里再写一次，确保无论从哪个入口
// 进来都是 production。
const buildInfoPath = path.join(repoRoot, 'dist', 'buildInfo.js');
mkdirSync(path.dirname(buildInfoPath), { recursive: true });
writeFileSync(
  buildInfoPath,
  '/** @generated by scripts/build-release.mjs —— 打包产物一律标记为 production。 */\n'
    + 'export const production = true;\n'
    + `export const channel = ${JSON.stringify(channel)};\n`,
);
console.log(`[build-release] buildInfo.js -> production=true channel=${channel}`);
