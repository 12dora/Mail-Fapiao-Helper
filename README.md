...
<details>
<summary>待确认队列堆了很多邮件，想让它们重新自动跑一遍</summary>

在 **待确认** 页点右上角的 **「全部重试」**。队列里每封邮件都保留着原始 `.eml`
副本，会按副本重新处理一遍：

- 这次能取到发票的，自动归档并移出队列
- 仍然失败的，留在队列里并更新失败原因

已经归档过的发票不会重复入库（按 `messageId + 来源 + 内容哈希` 幂等），
原始邮件也不会被删除，所以可以放心多点几次。

命令行等价写法：

```bash
node dist/index.js pending retry --config <config.json> --state <state.json>
```
</details>

<details>
<summary>通行费发票（票根网）的压缩包能正确处理吗</summary>

可以，两种投递格式都支持：

| 格式 | 结构 | 处理方式 |
|---|---|---|
| 扁平包 | `通行费电子发票.zip` 里直接放 `1_<纳税人识别号>_<uuid>.pdf/.ofd` | 逐条取出归档 |
| 包中包 | 外层每个开票方一个 `<纳税人识别号>_<uuid>.zip`，PDF/OFD/XML 在内层 | 下钻一层后取出归档 |

同一张票如果同时给了 `<stem>.pdf` 和 `<stem>.ofd`，只归档 PDF——两者是同一张
发票的两种格式，都收会让台账和识别队列翻倍。包内的 `.xml`（数电发票机读副本）
不归档，因为它不是可报销的票面文件。

随包附带的 **通行费电子票据汇总单**（票据 / 行程）会被识别为「支撑材料」，
照常归档但不进 OCR 队列。

> **v0.0.7 之前的版本**只解一层压缩包，「包中包」格式会一份票都取不到，而且同封
> 邮件里的汇总单归档成功会让整封邮件按"已完成"收尾——丢票在界面上看不出来。
> 升级后请到 **待确认** 页点「全部重试」，或对缓存邮件重跑一次「获取发票文件」。
</details>

<details>
<summary>开着代理（Surge / Clash / sing-box）时，几乎每封邮件都提示链接被拦截</summary>

这些工具默认开启 **fake-IP** 模式：DNS 把所有域名都映射到一段保留地址
（通常是 `198.18.0.0/15`），真实连接由 TUN / 代理还原。发票助手的 SSRF 防护
过去会把这类地址一律当成内网目标拒绝，于是每条发票直链都落进待确认。

现在应用会先用几个必然是公网的域名探测解析器：确认它在做映射之后，才放行
落在该占位段里的**域名解析结果**。回环、`10/8`、`172.16/12`、`192.168/16`、
`169.254/16`、CGNAT 等真正的内网段始终拒绝，URL 里写死的 IP 字面量也始终拒绝。

如果你希望完全关掉这个自动判断，设环境变量 `MFH_RESERVED_IP_POLICY=strict`。
</details>

<details>
<summary>发票库里混进了平台 logo、广告图、二维码</summary>

v0.0.7 之前，「是不是发票」这件事会看图片所在的**域名**。可开票平台的 CDN
同时也放自家的页眉、下载按钮、广告位、公众号二维码——它们和发票同域，于是
全被当成发票入库，还占着识别队列。

现在判据只看**路径**，并且先否决物料命名（logo / banner / ad / qrcode / ewm /
阅读器安装包……）；邮件里的 `multipart/related` 内联图（页眉、按钮、广告位）
按 MIME 语义识别为版式物料，只要同封邮件另有真发票文件就直接丢弃。

已经入库的那些不会自动消失。升级后到 **待确认** 页点「全部重试」，或对缓存邮件
重跑一次「获取发票文件」即可；旧的物料记录可以在发票库里手动删掉。
</details>

<details>
<summary>同一张票被归档了两份（一份 PDF、一份 OFD）</summary>

开票方常常在同一个压缩包 / 同一封邮件里同时给 PDF 和 OFD。旧版本只在两边文件名
都带 20 位发票号时才认得出是同一张票，通行费和携程这类用订单号命名的就会各存一份。

现在同一次投递里 `<相同词干>.pdf` + `<相同词干>.ofd` 只归档 PDF。已经重复入库的
历史数据用这条命令清理（关掉应用后运行）：

```bash
cd ~/Library/Application\ Support/mail-fapiao-helper
node <仓库路径>/dist/index.js dedupe --config ./config.json          # 先看会删什么
node <仓库路径>/dist/index.js dedupe --config ./config.json --apply  # 确认后执行
```

它只处理「同一封邮件、同一容器、同一非通用词干」的 PDF/OFD 配对，判据与归档时
完全一致；台账与磁盘对不上的行会被跳过并报出来；文件是**移进**
`invoices/.dedupe-quarantine/<时间戳>/` 而不是删除，确认无误后自行删掉该目录。
</details>
